...
struct cont_list
{
  ...
  union
  {
    ...
    struct
    {
      struct var_val_list *vvl;
    } CALL; // CALL表示退出当前环境后，要把rho置为vvl
  } d;
      ...
      case T_APP_R:
        ...
        case TV_BINOP_NAT:
          if (v->t == TV_NAT)
          {
            ...
            {
            case T_PLUS:
              nat = v->d.NAT.n + cl->d.APP_R.l->d.BINOP_NAT.n;
              break;
            case T_MINUS:
              nat = cl->d.APP_R.l->d.BINOP_NAT.n - v->d.NAT.n;
              break;
            case T_MUL:
              nat = cl->d.APP_R.l->d.BINOP_NAT.n * v->d.NAT.n;
              break;
        ...
        case TV_CLOS:
          if (cl->d.APP_R.l->d.CLOS.e->t == T_FUN_ABS)
          {
            if (cl->d.APP_R.l->d.CLOS.e->d.FUN_ABS.arg->t == T_FUN_ABS)
            {
              ...
              struct var_val_list *tmp;
              tmp = VVL_Cons(cl->d.APP_R.l->d.CLOS.e->d.FUN_ABS.name, r->d.VAL.val, cl->d.APP_R.l->d.CLOS.l);
              r->d.VAL.val = V_Clos(cl->d.APP_R.l->d.CLOS.e->d.FUN_ABS.arg, tmp);
              r->t = T_VAL;
              r->ectx = r->ectx->link;
            }
            else
